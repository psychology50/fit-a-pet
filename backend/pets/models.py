...
class ActiveImage(models.Model):
    image_id = models.BigAutoField(primary_key=True, unique=True, verbose_name="image_id")
    ...
    pet_id = models.ForeignKey(
        Pet, on_delete=models.CASCADE, db_column="pet_id", related_name="active_img"
    )
...
class Prescription(models.Model):
    prescription_id = models.BigAutoField(
        primary_key=True, unique=True, verbose_name="prescription_id"
    )
    hospital = models.CharField(max_length=45)
    content = models.TextField()
    create_dt = models.DateField()  # 진료 날짜 직접 입력
    pet_id = models.ForeignKey(
        Pet, on_delete=models.CASCADE, db_column="pet_id", related_name="prescription"
    )
...
class Event(models.Model):
    event_id = models.BigAutoField(primary_key=True, unique=True, verbose_name="event_id")
...
class Cycle(models.Model):
    cycle_id = models.BigAutoField(primary_key=True, unique=True, verbose_name="cycle_id")
    cycle_name = models.CharField(max_length=45)
    # is_notify = models.BooleanField(default=False)
    pet_id = models.ForeignKey(Pet, on_delete=models.CASCADE, db_column="pet_id")
    limit_at = models.CharField(max_length=45, default="0", blank=True, null=True)  # 추후 수정
    # 주기성 이벤트
...
class CycleDetail(models.Model):
    ...
    def __str__(self):
        return f"{self.detail_name}" if self.detail_name else f"{self.time}"


class AchieveCycle(models.Model):
    achieve_id = models.BigAutoField(primary_key=True, unique=True, verbose_name="achieve_id")
    date = models.DateTimeField(default=timezone.now, blank=True, null=True)
    detail_id = models.ForeignKey(
        CycleDetail, on_delete=models.CASCADE, db_column="detail_id", related_name="achievement"
    )
